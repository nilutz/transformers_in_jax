...
def test():
    ...
    bos_id = 2  # tokenizer.encode("<BOS>").ids[0]
    eos_id = 3  # tokenizer.encode("<EOS>").ids[0]
    pad_id = tokenizer.padding["pad_id"]  # tokenizer.encode("<PAD>").ids[0]
    print(bos_id, eos_id, pad_id)

    for src, tgt, src_mask, _ in test_dataloader:
        print(src.shape)
        print(src_mask.shape)

        encoded = apply_encode((src, src_mask))
        tgt_index = [[bos_id] * src.shape[0]]
